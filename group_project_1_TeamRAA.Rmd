...
#### Continuous values:
Plotting MSRP by year revealed a data quality issue, where vehicles manufactured prior to 1999 had a default value of $2000, which we believe would confound the true relationship of a vehicle's price and the age of the car.  We limited the data to cars manufactured on or after the year 2000.  Appendix 1.2
...
#### Outliers
There were two sets of outliers in the MSRP data.  As seen in 
...
#### Skewness:
Evaluating the MSRP data in Appendix 1.4 it is evident that the MSRP data is quite skewed and will have to be transformed for modeling.
...
After preparing the data for modeling, we evaluate the relationship of the data by doing a first pass MLR by MSRP considering our subset of attributes.  As observed by the pattern of the residuals on Appendix 1.7, they appear random and the QQ plot looks normally distributed.

Looking at the VIF data among variables, as displayed on Appendix 1.8, we can see that both Highway and City MPG Engine HP and Number of Doors all have a VIF greater than 2, so we will evaluate those for collinearity.
...
CaseData %>% ggplot(aes(x=Year, y=MSRP, group = Year, color=Driven_Wheels)) + geom_boxplot() + theme_dark()


```

#### 1.5 EDA: Popularity evaluation
...
#### 1.6 Summary of Data Before and After Impuding (Data Prep Continued)
```{r,echo = FALSE}
...
CaseDataPlot %>% ggplot(aes(x=Year, y=MSRP, group = Year, color=Driven_Wheels)) + geom_boxplot() + theme_dark() + facet_wrap(~Driven_Wheels)



myData %>% ggplot(aes(x=Year, y=MSRP, group = Year, color=Driven_Wheels)) + geom_boxplot() + theme_dark() + facet_wrap(~Driven_Wheels)
