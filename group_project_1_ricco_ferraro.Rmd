...
df_coeff.lambda.min.clean$estimate = df_coeff$`1`
df_coeff <- rename(df_coeff, estimate=`1`)
df_coeff <- rename(df_coeff, variable=rn)
df_coeff
df_coeff.clean <- df_coeff %>% filter(abs(estimate) > 0.000000e+00)
df_coeff.clean
```
```{r}
