...
glimpse(CaseData.NewCars.Filtered)

#Glimpse at the Engine.HP stats by Vehicle.Size
Vehicle.Size.Summary <- CaseData.NewCars.Filtered %>% filter(!isEmpty(Engine.HP)) %>% group_by(Vehicle.Size) %>% summarise(maxHP=max(Engine.HP), minHP=min(Engine.HP), medianHP=median(Engine.HP)) 

merge(CaseData.NewCars.Filtered, Vehicle.Size.Summary, by="Vehicle.Size") %>% mutate(if_else(isEmpty(Engine.HP),Engine.Hp = Vehicle.Size.Summary ) Engine.Hp=) -> CaseData.NewCars.Filtered
