...
Lasso
``` {r}
myData <- CaseData.NewCars.Filtered.ImputedHPAndCylinders[, c('Vehicle.Size','Year', 'Engine.Fuel.Type', 'Transmission.Type', 'Driven_Wheels', 'Vehicle.Style', 'highway.MPG', 'city.mpg', 'Popularity', 'Engine.HP.Clean', 'Engine.Cylinders.Clean', 'NumDoors.Clean', 'MSRP')]
myData <- myData %>% filter(Year > 2000)
...
x_train=model.matrix(MSRP~.,train)[, -1]
y_train=log(train$MSRP)
x_test<-model.matrix(MSRP~.,test)[, -1]
...
tempModel <- lm(log(MSRP)~., data=myData)
plot(tempModel)
coefplot(tempModel)
